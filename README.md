...
```bash
cloud-resume1$ sam build --use-container
```
...
```bash
cloud-resume1$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
cloud-resume1$ sam local start-api
cloud-resume1$ curl http://localhost:3000/
```
...
```bash
cloud-resume1$ sam logs -n HelloWorldFunction --stack-name "cloud-resume1" --tail
```
...
```bash
cloud-resume1$ pip install -r tests/requirements.txt --user
# unit test
cloud-resume1$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
cloud-resume1$ AWS_SAM_STACK_NAME="cloud-resume1" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "cloud-resume1"
```
